...
The idea for axx, the name 'AXX', and the prototype written in C already existed in 1986, when I was working part-time at Tokyo Electronics Design during my university days. However, it wasn't until 2024, 38 years later, that I published working code like the one we see today. It was dormant for 30 years. The `instruction` in the axx pattern file is the meta-language for all imperative assembly languages. Although it's a DSL, it doesn't really have a defined grammar; it's a free language (pattern language) where you create your own grammar by combining string literals, symbols, and expressions.
