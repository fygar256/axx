...
axx.py is a general assembler that generalizes assembly language. It can process almost any processor architecture. To process each processor architecture, a corresponding pattern file (processor description file) is required. While you can define any instructions, creating a pattern file based on the target processor's assembly language will allow it to process that processor's assembly language, albeit with slightly different syntax. Essentially, all it requires is instruction grammar rules and binary generation based on those rules.

It is not a "general assembler" in the sense of being "widely applicable." It is a "general assembler" in the sense of being "common to all" processors except those with meta-level complexity. In other words, axx.py can adapt to processors with complex architectures, but it cannot support some EPIC processors that have meta-level bundles of instructions. Therefore, it is a "general assembler," not a "universal assembler." Pattern data only has control syntax: assignment, ternary operator, `;` modifier, and alignment. It can be used to generate binaries other than assembly language. axx.py can handle any processor where there is a one-to-one mapping between instructions and machine codes. Pattern files are Turing-incomplete. Because they are Turing-incomplete, they are not suitable for processors with very twisted architectures. Processor architectures can become infinitely complex if you want them to be. If they were Turing-complete, they could be followed, but because axx.py is Turing-incomplete, it is not a "universal assembler."
...
axx reads assembler pattern data from the first argument, and assembles the source file of the second argument based on the pattern data. At that time, the pattern data is matched against each assembly line line by line from the top, and a binary_list of matched patterns is output as the result. Patterns are represented by arbitrary string constants, arbitary numerized string constants, arbitary integer values, and expression evaluation involving floating point numbers, so any assembly language can be processed. If the second argument is omitted, the source is input from the terminal (standard input).
...
```
This example processes an x86_64 assembly line like `leaq rax,[rax+rbx*2+0x40]`, as shown below. If you have needed to use escape characters in parentheses within them, use !! to evaluate the factor.
