...
両実装とも、組み込みの表（`ELF_MACHINES`）は読み取り専用の土台のままにして、
その上に宣言を重ねた**実効表**を 1 つ作り、型を引くところは全部そこを通します。

- `axx.py` : `elf_machine_table(state)`
- `caxx.c` : `elf_machine_effective(st)`

実効表は「宣言の版（`decl_gen`）とマシン番号」を鍵に控えます。宣言はパターン行
として 1 ソース行ごとに通りますが、中身が変わったときだけ版が進むので、組み立て
中に表を組み直すことはありません。

名前の並びは両実装で同じです。

1. 組み込みの名前のうち、`.elftype` で同じ綴りを宣言していないもの
2. `.elftype` の宣言（宣言順）

名前引き・逆引き（`-E` の書き出し）・幅引きはどれも先頭から探します。この並びが
同じであることが、2 つの実装の出力が一致する条件です。

### 6.2 宣言の登録

`register_elftypes()` を `register_elfdecls()` に広げ、8 つの宣言をすべて組み立て
前に一度登録します。ソースの `.extern` / `.global` / `.EQU` / `.RELOCTYPE` や
取り込み TSV はパターン表をたどるより前に読まれるので、ここでそろえておかないと
「まだ宣言されていない」ことになります。登録の直後に `check_elfdecls()` が型欄を
引けるか見ます。

### 6.3 ついでに直したところ

AArch64 の命令フィールド型の表（`insn_reloc_field_mask`）を、対象が AArch64 の
ときだけ引くようにしました。型番号の意味はマシンごとに違うので、`.elftype` で
275 番を宣言した別機種の型を ADRP と取り違えないためです。
...
| `axx.py` | Python 実装 |
| `caxx.c` | C 実装 |
| `elfgen.axx` | EM_MSP430 の ELF 記述（テスト用パターンファイル） |
| `elfgen.s` | その対のソース |
| `test1` | `elfgen.axx` / `elfgen.s` の `-o` 出力を両実装で比較（24 組目） |
| `Technical_Manual_ja.md` | 3.7.7 節（新設）、3.7.6 節、2.2 節、オプション表 |
| `Technical_Manual.md` | 同上（英語） |
| `FILE_DESCRIPTION` | `elfgen.axx` / `elfgen.s` の説明 |
| `axx.1.gz` | `-m` / `-f` の説明 |

## 8. 確認したこと

- `test1`（24 組）が両実装で `test all passed`。
- 今回の変更の前後で、既存の 23 組の出力がバイト単位で同一。
- x86-64 / AArch64 の `-o`（`-g` 付きを含む）も、変更前の実装とバイト単位で同一。
- `elfgen` の `.o` は Paxx と Caxx でバイト単位で同一。`readelf` がリロケーション
  名・ヘッダ欄を上のとおりに読む。
- 誤りのある宣言に対する診断が、両実装で 1 文字違わず同じ。
