...
`.echo` is the debugging counterpart: it writes to stderr and appends nothing
to the output, so adding or removing one never changes the generated bytes.
Each item is either a `"..."` string literal or an expression, and the two may
be mixed:
...
`[1, 2, 3]`, and the items of one call share a line separated by spaces.
`.echo()` with no item prints an empty line. Strings may appear only in the
argument list of `.echo` — values are integers and arrays only, so a string
cannot be assigned or used in an expression. The escapes are `\\`, `\"`, `\n`
and `\t`; any other `\` is an error, and a string cannot contain `/*` — that is
the pattern file's comment marker, stripped before the mini language sees the
line. Nothing is printed while the assembler is
still measuring instruction lengths or converging in pass 1, so each line
appears once per assembled instruction, not once per relaxation iteration.
...
### 6.3 Very large values and the UNDEF sentinel
