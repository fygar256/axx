...
In relation to Sections 4 and 5.2, termination and convergence are now secured at three levels. The pattern layer is secured as a property of the language specification (through Turing incompleteness), an individual macro expansion by resource limits, and the *sequence* of expansions by the relaxation fixed point. That the first two are static guarantees while only the third is run-time detection is an asymmetry in the design worth stating explicitly. The earlier restriction on references was the choice that obtained this third one for free; lifting it was possible only because the run-time detection mechanism was already in operation beforehand.
